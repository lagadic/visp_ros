...
//function for transaltion motion
...
vpHomogeneousMatrix getMatrixPush2(const std::vector<double>& Box1, const std::vector<double>& Box2, const std::vector<double>& Box3, double x, double y, double z) {
    ...
    // target position
    double X = x;
    double Y = Box1[1] + 0.5*Box3[1] - delta;
    double Z = Box3[2] + delta;
...
//function for rotate-push motion
...
vpHomogeneousMatrix getMatrixY1(const std::vector<double>& Box1, const std::vector<double>& Box2, double x, double y, double z) {//go down, then do push
    std::cout << "Going down" << std::endl;

    // factor1 is used for deeper push because of the limitation of physical engine and heavy parcel, you may change it according to the parcel you want
    double factor1 = 1.1;
...
vpHomogeneousMatrix getMatrixY2(const std::vector<double>& Box1, const std::vector<double>& Box2, double x, double y, double z) {
    std::cout << "go up with rotating part of theta (20%)" << std::endl;
    ...
    //print Rotation angle and HomogeneousMatrix
    std::cout << "Rotation angle: " << theta_half << std::endl;
...
vpHomogeneousMatrix getMatrixY3(const std::vector<double>& Box1, const std::vector<double>& Box2, double x, double y, double z) {
    ...
    // target position
    double X = 0.5*Box2[0]*0.98;	// multiplied by a number smaller than 1 to be closer to the side
    double Y = 0.5*Box2[1]*1.1; 	// multiplied by a number bigger than 1, otherwise it may hit the side of tote
    double Z = Box2[2]*c_half + 0.5*Box2[1]*s_half;
    
    // rotating motion Y3, step 3
    vpHomogeneousMatrix fM_Y3_rotate(vpTranslationVector(X, Y, factor3*Z),
                                     vpRotationMatrix( {1,0,0,0,1,0,0,0,1} ) );
...
vpHomogeneousMatrix getMatrixY4(const std::vector<double>& Box1, const std::vector<double>& Box2, double x, double y, double z) {
    ...
    double Y = 0.5*Box2[1]*1.2;	// multiplied by some values otherwise it crash the side of tote
...
vpHomogeneousMatrix getMatrixY5(const std::vector<double>& Box1, const std::vector<double>& Box2, double x, double y, double z) {
    std::cout << "after going down, do translation motion in y-direction and place the parcel" << std::endl;
    ...
    // target position
    double X = 0.365 - (Box1[0] + 0.5*Box2[0]); //0.365 is the width of tote
    double Y = 0.5*Box2[1]*1.2;	// multiplied by some values otherwise it crash the side of tote
    double Z = Box2[2] + 0.8*delta;	// added by some values otherwise it crash the bottom of tote
...
// initialize force and torque to 0.0
double force_x = 0.0;
double force_y = 0.0;
double force_z = 0.0;
double torque_x = 0.0;
...
// initialize a ColVector to save values of the force-torque sensor
...
int main( int argc, char **argv )
{
	...
	// connect Coppeliasim
        ...
        vpHomogeneousMatrix fM_eed_home(vpTranslationVector(0, -0.20, 0.2), 
                                        vpRotationMatrix( {1, 0, 0, 0, 1, 0, 0, 0, 1} ) );
                                        
        // top of left tote
        vpHomogeneousMatrix fM_eed_l_tote(vpTranslationVector(0.3, 0.3, 0.15),
                                          vpRotationMatrix( {0, 1, 0, 1, 0, 0, 0, 0, -1} ) );
                                          
        // top of conveyor
        vpHomogeneousMatrix fM_eed_ConveyorTop(vpTranslationVector(-0.35, 0.2, 0.15),
                                          vpRotationMatrix( {0, 1, 0, 1, 0, 0, 0, 0, -1} ) );

        // origin of left tote in robot base frame
        vpHomogeneousMatrix fM_origin_l_tote(vpTranslationVector(0.2002, -0.0124, -0.4225),
                                      vpRotationMatrix( {1, 0, 0, 0, 1, 0, 0, 0, 1} ) );
	
	// initialize active_cdMc
	...
	// vector of all objects in the scene w.r.t. the world frame
        ...
        // save the size of Boxes and the tote
        ...
        while ( !final_quit )
        {
            ...
            // back to top of the conveyor, interpolating in order not to hit the bench
            else if( State == 12 ){
                active_cdMc = (fM_eed_ConveyorTop*eMc).inverse()*robot.get_fMe()*eMc ;
                t.buildFrom(active_cdMc);
                tu.buildFrom(active_cdMc);
            ...
            // Go to target position over Box1[2] length, prepare for translation push (1)
            else if( State == 14 ){
		...
		active_cdMc = (fM_origin_l_tote* fM_Push1*edMo.inverse()*eMc).inverse()*robot.get_fMe()*eMc ;
            ...
            // Go to target position above by 0.15 meters without any tilt angle
            else if( State == 4 ){
                // use fM_origin_l_tote.inverse()*obj_vec[0].wMo)[0] to obtain new position of Box1 after translation motion
            ...
            // Going down with rotating
                ...
                active_cdMc = (fM_origin_l_tote*fM_Y1_rotate*edMo.inverse()*eMc).inverse()*robot.get_fMe()*eMc ; 
                t.buildFrom(active_cdMc);
                tu.buildFrom(active_cdMc);
                if ( !servo_started )
                {
            ...
            //go up without rotating
                ...
                active_cdMc = (fM_origin_l_tote* fM_Y2_rotate*edMo.inverse()*eMc).inverse()*robot.get_fMe()*eMc ;
                t.buildFrom(active_cdMc);
                tu.buildFrom(active_cdMc);
                if ( !servo_started )
                {
        	...
        	vpHomogeneousMatrix fM_Y3_rotate = getMatrixY3(Box[0], Box[1], (fM_origin_l_tote.inverse()*obj_vec[0].wMo)[0][3], (fM_origin_l_tote.inverse()*obj_vec[0].wMo)[1][3], (fM_origin_l_tote.inverse()*obj_vec[0].wMo)[2][3]);
                active_cdMc = (fM_origin_l_tote* fM_Y3_rotate*edMo.inverse()*eMc).inverse()*robot.get_fMe()*eMc ;
                t.buildFrom(active_cdMc);
                tu.buildFrom(active_cdMc);
                if ( !servo_started )
                {
            ...
            // do translation motion again in y-direction using force sensor
            else if( State == 19 ){
	    ...
	    // error for length and angle
            double error_tr  = sqrt( active_cdMc.getTranslationVector().sumSquare() );           
            double error_tu  = vpMath::deg( sqrt( active_cdMc.getThetaUVector().sumSquare() ) ); 

            // FSM transition conditions
            if(error_tr <= 1 && error_tu <= 5 && State == 0){ // once reached the position above the right tote, move to the left tote

                std::cout << "go to the left tote for interpolation\n";
                State = 22;
                servo_started = false;

            }else if(error_tr <= 0.1 && error_tu <= 5 && State == 22)// once reached the top of conveyor, go to pick the parcel on the conveyor
            {
                std::cout << "top of conveyor reached... moving to the top of left tote \n";
            ...
            }else if(error_tr <= 0.1 && error_tu <= 1 && State == 12){ // once picked the parcel, move to the left tote
            ...
            }else if(error_tr <= 0.05 && error_tu <= 2 && State == 14){ 
                std::cout << "prepare for translation push 2\n";
                State = 15;
                servo_started = false;
                
            }
            else if(error_tr <= 0.01 && error_tu <= 2 && State == 15){ 

                std::cout << "translating in x-direction\n";
                State = 16;
                servo_started = false;

            }
            else if(((error_tr <= 0.001 && error_tu <= 1) || abs(ft_sensor[1]) >= 4 )  && State == 16){ // stop based on force sensor, you can add "|| abs(ft_sensor[0]>= 3)" for more oriented control

                std::cout << "placing in y-direction\n";
                State = 17;
                servo_started = false;

            }else if((error_tr <= 0.001 && error_tu <= 1) && State == 17){ // once you have placed the box, go to top of left tote
            ...
            }else if(error_tr <= 0.2 && error_tu <= 1 && State == 18){ // once you have placed the box, go to top of conveyor
	    ...
	    // picking next box and do rotation
            ...
            else if((error_tr <= 0.001 && error_tu <= 0.5 || abs(ft_sensor[0]) >= 3 ) && State == 19){ // once you have reached the side, go to top of left tote	//|| abs(ft_sensor[0]) >= 3) 
            ...
            else if((error_tr <= 0.001 && error_tu <= 0.5 || abs(ft_sensor[1]) >= 3) && State == 21){ // once you have reached the side, go to top of left tote		//|| abs(ft_sensor[0]) >= 3) 
            ...
            v_c = task.computeControlLaw() - v_0*exp(-8.0*(robot.getCoppeliasimSimulationTime() - sim_time_init_servo));
