...
vpHomogeneousMatrix getMatrixPush2(const std::vector<double>& Box1, const std::vector<double>& Box2, const std::vector<double>& Box3, double x, double y, double z) {//translating
    ...
    // target position
    double X = x;
    double Y = Box1[1] + 0.5*Box3[1] - 1*delta;
    double Z = Box3[2] + delta;
...
int main( int argc, char **argv )
{
	...
	//suction grabber
        ...
        // interpolation point at beginning
        vpHomogeneousMatrix fM_interpolation_0(vpTranslationVector(-0.32, -0.43, 0.15),
                                               vpRotationMatrix( {0, 1, 0, 1, 0, 0, 0, 0, -1} ) );
        ...
        // top of conveyor, used at the beginning
        //vpHomogeneousMatrix fM_eed_ConveyorTop0(vpTranslationVector(-0.22, -0.2, 0.15),
                                          //vpRotationMatrix( {0, 1, 0, 1, 0, 0, 0, 0, -1} ) );
        ...
        vpHomogeneousMatrix fM_origin_l_tote(vpTranslationVector(0.2002, -0.0124, -0.4225),//-0.046235, -0.23424, -0.4225
                                      vpRotationMatrix( {1, 0, 0, 0, 1, 0, 0, 0, 1} ) );//{0, -1, 0, 1, 0, 0, 0, 0, 1}
	...
	// vector of all objects in the scene w.r.t. the world frame
		...
		active_cdMc = (fM_origin_l_tote* fM_Push1*edMo.inverse()*eMc).inverse()*robot.get_fMe()*eMc ;
            ...
            else if(((error_tr <= 0.001 && error_tu <= 1) || abs(ft_sensor[1]) >= 4 )  && State == 16){ // stop based on force sensor || abs(ft_sensor[0]>= 3)

                std::cout << "placing in y-direction\n";
                State = 17;
                servo_started = false;

            }else if((error_tr <= 0.001 && error_tu <= 1) && State == 17){ // once you have placed the box, go to top of left tote	//abs(ft_sensor[1]) >= 1.5 |||| abs(ft_sensor[0]) >= 10
