...
class RosViper650Node : public rclcpp::Node
{
public:
  RosViper650Node();
...
protected:
  ...
  int m_tool_type; // See https://visp-doc.inria.fr/doxygen/visp-daily/classvpViper650.html structure vpToolType
  std::string m_tool_custom_transformation_file;

  rclcpp::Time m_vel_time;

  vpRobotViper650 *m_robot;
...
RosViper650Node::RosViper650Node()
  ...
  , m_queue_size( 10 )
{
  RCLCPP_INFO( this->get_logger(), "Using Viper650 robot" );
  ...
  {
    RCLCPP_INFO( this->get_logger(), "Viper650 robot controlled in joint space" );
    ...
    m_cmd_jointvel_sub = this->create_subscription< sensor_msgs::msg::JointState >(
        m_cmd_vel_topic_name, m_queue_size, std::bind( &RosViper650Node::setJointVel, this, std::placeholders::_1 ) );
  }
  else if ( m_control_mode == "camera_frame" )
  {
    RCLCPP_INFO( this->get_logger(), "Viper650 robot controlled in camera frame" );
    // Create publishers
    ...
    m_cmd_camvel_sub = this->create_subscription< geometry_msgs::msg::TwistStamped >(
        m_cmd_vel_topic_name, m_queue_size, std::bind( &RosViper650Node::setCameraVel, this, std::placeholders::_1 ) );
  }
  else if ( m_control_mode == "reference_frame" )
  {
    RCLCPP_INFO( this->get_logger(), "Viper650 robot controlled in reference frame" );
    // Create publishers
    m_pose_pub = this->create_publisher< geometry_msgs::msg::PoseStamped >( "pose", m_queue_size );
    m_vel_pub  = this->create_publisher< geometry_msgs::msg::TwistStamped >( "velocity", m_queue_size );
    ...
    m_cmd_refvel_sub = this->create_subscription< geometry_msgs::msg::TwistStamped >(
        m_cmd_vel_topic_name, m_queue_size, std::bind( &RosViper650Node::setRefVel, this, std::placeholders::_1 ) );
  }
}
...
{
  m_robot = new vpRobotViper650;

  if ( m_tool_type == (int)( vpViper650::TOOL_PTGREY_FLEA2_CAMERA ) )
  {
    m_robot->init( vpViper650::TOOL_PTGREY_FLEA2_CAMERA, vpCameraParameters::perspectiveProjWithDistortion );
  }
  else if ( m_tool_type == (int)( vpViper650::TOOL_GENERIC_CAMERA ) )
  {
    m_robot->init( vpViper650::TOOL_GENERIC_CAMERA, vpCameraParameters::perspectiveProjWithDistortion );
  }
  else if ( m_tool_type == (int)( vpViper650::TOOL_CUSTOM ) )
  {
    if ( vpIoTools::checkFilename( m_tool_custom_transformation_file ) == false )
    {
      RCLCPP_ERROR( this->get_logger(), "Viper650: Missing or bad filename for eMt custom tool transformation" );
      return -1;
    }
    ...
    {
      RCLCPP_ERROR( this->get_logger(), "Viper650: Cannot load eMt custom tool transformation from \"%s\"",
                    e.getMessage() );
      f.close();
      return -1;
    }

    m_robot->init( vpViper650::TOOL_CUSTOM, eMt );
  }
...
RosViper650Node::publish()
{
  ...
  else if ( m_state_mode == "camera_frame" )
  {
    ...
    // RCLCPP_INFO( this->get_logger(), "Viper650 publish pose at %f s: [%0.2f %0.2f %0.2f] - [%0.2f %0.2f %0.2f
    // %0.2f]",
...
void
RosViper650Node::setCameraVel( const geometry_msgs::msg::TwistStamped::ConstSharedPtr &msg )
{
  ...
  // RCLCPP_INFO( this->get_logger(), "Viper650 new camera vel: [%0.2f %0.2f %0.2f] m/s [%0.2f %0.2f %0.2f] rad/s",
  // vc[0],
...
void
RosViper650Node::setJointVel( const sensor_msgs::msg::JointState::ConstSharedPtr &msg )
{
  m_vel_time = rclcpp::Node::now();

  if ( msg->velocity.size() != 6 )
  {
    RCLCPP_ERROR( this->get_logger(), "Viper650: Cannot apply a joint velocity vector that is not 6 dimensional" );
    return;
  }
  ...
  // RCLCPP_INFO( this->get_logger(), "Viper650 new joint vel: [%0.2f %0.2f %0.2f %0.2f %0.2f %0.2f] rad/s", qdot[0],
  //              qdot[1], qdot[2], qdot[3], qdot[4], qdot[5] );
  m_robot->setVelocity( vpRobot::ARTICULAR_FRAME, qdot );
}

void
RosViper650Node::setRefVel( const geometry_msgs::msg::TwistStamped::ConstSharedPtr &msg )
{
  m_vel_time = rclcpp::Node::now();
  ...
  // RCLCPP_INFO( this->get_logger(), "Viper650 new reference vel: [%0.2f %0.2f %0.2f] m/s [%0.2f %0.2f %0.2f] rad/s",
...
main( int argc, char **argv )
{
  ...
  auto node = std::make_shared< RosViper650Node >();

  try
  {
    if ( node->setup() != 0 )
    {
      RCLCPP_ERROR( node->get_logger(), "Viper650 setup failed... \n" );
      return EXIT_FAILURE;
    }
  ...
  std::cout << "This node is not available since ViSP was" << std::endl
            << "not build with Viper650 robot support...\n"
            << std::endl;
#endif
  return EXIT_SUCCESS;
}
