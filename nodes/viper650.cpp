...
RosViper650Node::RosViper650Node(ros::NodeHandle nh)
{
  ...
  ROS_INFO("Launch Viper650 robot ros node");
...
//  jointState_pub = n.advertise<sensor_msgs::JointState>("joint_state", 1000);
  ...
  if (setControlMode == "joint_space") {
    ROS_INFO("Viper650 robot controlled in joint space");
    ...
    cmd_jointvel_sub = n.subscribe( cmdVelTopicName, 1, (boost::function < void(const sensor_msgs::JointStateConstPtr&)>) boost::bind( &RosViper650Node::setJointVel, this, _1 ));
  }
  else if (setControlMode == "camera_frame") {
    ROS_INFO("Viper650 robot controlled in camera frame");
    pose_pub = n.advertise<geometry_msgs::PoseStamped>("pose", 10);
    vel_pub = n.advertise<geometry_msgs::TwistStamped>("velocity", 10);
    cmd_camvel_sub = n.subscribe( cmdVelTopicName, 1, (boost::function < void(const geometry_msgs::TwistStampedConstPtr&)>) boost::bind( &RosViper650Node::setCameraVel, this, _1 ));
  }
  else if (setControlMode == "reference_frame") {
    ROS_INFO("Viper650 robot controlled in reference frame");
    pose_pub = n.advertise<geometry_msgs::PoseStamped>("pose", 10);
    vel_pub = n.advertise<geometry_msgs::TwistStamped>("velocity", 10);
    cmd_refvel_sub = n.subscribe( cmdVelTopicName, 1, (boost::function < void(const geometry_msgs::TwistStampedConstPtr&)>) boost::bind( &RosViper650Node::setRefVel, this, _1 ));
  }
}
...
int RosViper650Node::setup()
{
  ...
  if (endEffectorType == "TOOL_PTGREY_FLEA2_CAMERA") {
    robot->init(vpViper650::TOOL_PTGREY_FLEA2_CAMERA, vpCameraParameters::perspectiveProjWithDistortion);
  }
    ...
    if (vpIoTools::checkFilename(customToolTransformationFileName) == false) {
      ROS_ERROR("Viper650: Missing or bad filename for eMt custom tool transformation");
      return -1;
    }
    ...
    try {
      ...
      ROS_ERROR_STREAM("Viper650: Cannot load eMt custom tool transformation from \"" << customToolTransformationFileName << "\": " << e.getStringMessage());
      f.close();
      return -1;
    }
...
void
RosViper650Node::setJointVel(const sensor_msgs::JointStateConstPtr &msg)
{
  veltime = ros::Time::now();

  if (msg->velocity.size() != 6) {
    ROS_ERROR("Viper650: Cannot apply a joint velocity vector that is not 6 dimensional");
    return;
  }
  ...
  ROS_INFO("Viper650 new joint vel at %f s: [%0.2f %0.2f %0.2f %0.2f %0.2f %0.2f] rad/s",
           veltime.toSec(),
...
void
RosViper650Node::setRefVel(const geometry_msgs::TwistStampedConstPtr &msg)
{
  veltime = ros::Time::now();
  ...
  ROS_INFO( "Viper650 new reference vel at %f s: [%0.2f %0.2f %0.2f] m/s [%0.2f %0.2f %0.2f] rad/s",
            veltime.toSec(),
